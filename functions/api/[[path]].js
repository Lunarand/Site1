export async function onRequest(context) {      
  ...
  try {      
    ...
    if (method === "POST" && path === "upload") {      
      ...
      // ---------- NEW: Firebase file URLs support ----------
      // Frontend can upload to Firebase Storage and send URLs here as "firebaseFiles"
      const firebaseFiles = form.getAll("firebaseFiles").filter(Boolean);
      // Also support an alternate field name just in case (doesn't break anything)
      ...
      // ---------- NEW: Add Firebase URLs into attachments ----------
      // These are already public URLs from Firebase Storage
      // We store them as-is, no KV file storage used.
...
// ---------- NEW: best-effort guessing from URL (Firebase Storage download URLs)
function guessFromUrl(urlStr) {
  try {
    const u = new URL(urlStr);
    const pathname = u.pathname || "";
    const rawName = pathname.split("/").pop() || "file";
    const name = decodeURIComponent(rawName.split("?")[0] || rawName || "file");
...
// ---------- NEW: minimal extension -> mimetype mapping
